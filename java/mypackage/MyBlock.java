...
public class MyBlock extends Block {
	...
	public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
		...
		for (Drop d : drops) {
			if (d.item != null) result.add(new ItemStack(d.item,random.nextInt(d.max-d.min+1)+d.min));
			else result.add(new ItemStack(Item.getItemFromBlock(d.block),random.nextInt(d.max-d.min+1)+d.min));
		}
		return result;
	}
